...
# --- timeframe config -------------------------------------------------
# Yahoo limits intraday history: 2m/5m bars go back only ~60 days.
# Pass an interval as the 2nd arg:  python3 markov_intraday.py ORIC 2m
INTERVAL = "2m"          # default; override on the command line
PERIOD_FOR = {"1m": "7d", "2m": "60d", "5m": "60d", "15m": "60d",
              "30m": "60d", "60m": "2y", "1h": "2y", "1d": "2y"}
# For intraday, the "gap" is the OVERNIGHT jump into the first bar of a
# session; within a session there is no gap. We also relax the threshold
# because intraday moves are smaller than daily ones.
GAP_THRESHOLD = 0.02
SMA_LEN = 20
...
HISTORY_FILE = "markov_intraday_history.json"
...
def label_states(df):
    ...
    slope = sma.diff()
    # bar-to-bar open vs previous close; intraday, the large ones are
    # overnight session boundaries (the intraday "catalyst gaps").
...
def analyze(t, interval=None):
    interval = interval or INTERVAL
    df = get_bars(t, interval)
    s = label_states(df)
    ...
    dur = {x: (1 / (1 - p.loc[x, x])
           ...
           for x in STATES}
    return {"ticker": t, "n_days": len(s), "interval": interval,
...
def to_record(res):
    ...
    return {
        ...
        "interval": res.get("interval", INTERVAL),
...
def history_page(hist):
    ...
    return page("Run History",
                ...
                summary + blocks,
                nav='<a href="markov_intraday.html">&larr; Latest run</a>')
...
def page(title, subtitle, body, nav=""):
    return (f'<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">'
            ...
            f'<div class="eyebrow">Regime study · {INTERVAL} intraday bars · 20-SMA + 2% gaps</div>'
...
# ---------------------------------------------------------------- main
def build(tickers, interval=None):
    interval = interval or INTERVAL
    cards, records = "", []
    for t in tickers:
        print(f"Analyzing {t} @ {interval}...")
        try:
            rec = to_record(analyze(t, interval))
            cards += card(rec)
            records.append(rec)
        except Exception as e:
            print(f"  skipped {t}: {e}")
    ...
    with open("markov_intraday.html", "w") as f:
        f.write(page("Markov Regime Dashboard",
                     ...
                     nav='<a href="markov_intraday_history.html">Run history &rarr;</a>'))

    hist = save_history(records)
    with open("markov_intraday_history.html", "w") as f:
        f.write(history_page(hist))

    print(f"\nWrote markov_intraday.html  (this run: {', '.join(r['ticker'] for r in records)})")
    print(f"Wrote markov_intraday_history.html  ({len(hist)} runs logged)")
    print("Open with:  open markov_intraday.html")
...
if __name__ == "__main__":
    ...
    tickers = []
    for a in raw:
        if a.lower() in PERIOD_FOR:      # an interval like 2m / 5m / 15m
            interval = a.lower()
        else:
            tickers.append(a.upper())
    globals()["INTERVAL"] = interval
    build(tickers or ["ORIC"], interval)
